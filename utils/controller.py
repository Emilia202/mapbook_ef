...
def get_coordinate(location_name: str) -> list:
    import requests
...
def get_map(users_data: list[dict]) -> None:
    import folium
    map = folium.Map(location=[52.3,21.0], zoom_start=6, titles='OpenStreetMap')
    for user in users_data:
        print(get_coordinate(user['location']))
        folium.Marker(
            location=get_coordinate(user['location']),
